...
//    iddent | nom | couttraitement | coutremplacement | coutnettoyage | coutenlevement | ordrenumerique | positiongd | positionhb | typepriorite
    ...
    public Dent[] getAllDent(Connection c) throws Exception {
        ...
        ResultSet res = st.executeQuery("select * from dent order by idDent asc");
    ...
    public static void insertSpecialAlea(Connection co, int idSoin,String test,String etatTest) throws Exception{
        String[] nify = null;
        if(test.contains("-")){
               nify =  test.split("-");
              for (int i = Integer.valueOf(nify[0]); i <= Integer.valueOf(nify[1]); i++) {
                  Dent.insertEtatDentPatient(co,idSoin,i,Integer.valueOf(etatTest));
                  System.out.println("Test.Main.main()");
              }
          }else if(test.contains(";")){
               nify =  test.split(";");
               String[] etatpartiel = etatTest.split(";");
              for (int i = 0; i < nify.length; i++) {
                  Dent.insertEtatDentPatient(co, idSoin, Integer.valueOf(nify[i]), Integer.valueOf(etatpartiel[i]));
              }
          }
    }
       public static void insertSpecialAlea2(Connection co, int idSoin,String test,String etatTest,Dent d) throws Exception{
        String[] nify = null;
        if(test.contains("-")){
               nify =  test.split("-");
               if(String.valueOf(nify[0])==null){
               for (int i =0;i <= Integer.valueOf(nify[1]);i++) {
                  Dent.insertEtatDentPatient(co,idSoin,i,Integer.valueOf(etatTest));
                  System.out.println("Test.Main.main()");
              }
               }
